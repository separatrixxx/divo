...
export async function getUser(args: GetUserArguments) {
    ...
    try {
        ...
        if (err.response && err.response.data.error_message === 'User not found') {
            webApp?.showAlert(setLocale(router.locale).errors.user_not_found_error, async function() {
                router.push('/');
            }); 
        } else {
            webApp?.showAlert(setLocale(router.locale).errors.get_user_error, async function() {
                router.push('/');
            }); 
        }
